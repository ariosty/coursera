...
class Apocalypse(poc_grid.Grid):
    ...
    def compute_distance_field(self, entity_type):
        ...
        distance_field = [[max_dist for dummy_col in range(self._grid_width)]
        ...
        while (boundary.__len__() > 0):
            ...
            for neighbor in visited.four_neighbors(current_cell[0], current_cell[1]):
                if (visited.is_empty(neighbor[0], neighbor[1]) and
                    self._cells[neighbor[0]][neighbor[1]] != FULL):
    ...
    def move_humans(self, zombie_distance_field):
        ...
        for idx in range(len(self._human_list)):
            ...
            target_list = []
            target_dist = zombie_distance_field[human[0]][human[1]]
            for cell in neighbors:
                if (zombie_distance_field[cell[0]][cell[1]] > target_dist and
                    poc_grid.Grid.is_empty(self, cell[0], cell[1])):
                    target_dist = zombie_distance_field[cell[0]][cell[1]]
            neighbors.append(human)
            for cell in neighbors:
                if (target_dist == zombie_distance_field[cell[0]][cell[1]] and
                    self._cells[cell[0]][cell[1]] != FULL):
    ...
    def move_zombies(self, human_distance_field):
        ...
        for idx in range(len(self._zombie_list)):
            ...
            for cell in neighbors:
                if (target_dist == human_distance_field[cell[0]][cell[1]] and
                    self._cells[cell[0]][cell[1]] != FULL):
                    target_list.append(cell)
            target = random.choice(target_list)
